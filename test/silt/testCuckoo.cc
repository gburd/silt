...
namespace silt
{
    ...
    class Silt_Cuckoo_Test : public testing::Test
    {
    public:
        Silt *h;
    ...
    protected:
        virtual void SetUp() {
            ...
            h = Silt_Factory::New(conf_file); // test_num_records_
            Silt_Return r = h->Create();
			assert(r == OK);
    ...
    void test_put_get(uint32_t num, Silt *&h, bool stop_if_full = false) {
        char *key;
        char *val;
        ...
        for (uint32_t i = 0; i < num; i++) {
            key = (char*) &(key_vec1[i]);
            val = (char*) &(val_vec1[i]);
			Silt_Return ret = h->Put(ConstRefValue(key, 8), ConstRefValue(val, 4));
			if (stop_if_full) {
    ...
    void test_loadfactor(Silt *&h) {
        ...
        while (round <= MAXROUND) {
            key = (char*) &(key_vec1[i]);
            val = (char*) &(val_vec1[i]);
            Silt_Return r = h->Put(ConstRefValue(key, 8), ConstRefValue(val, 4));
            if (r == OK)
                count ++;
            else{
                Value table_size;
    ...
    TEST_F(Silt_Cuckoo_Test, SimpleTest_PutGet1Key) {
        test_put_get(1, h);
    }

    TEST_F(Silt_Cuckoo_Test, SimpleTest_PutGet1000Keys) {
        test_put_get(1000, h);
    }

    TEST_F(Silt_Cuckoo_Test, SimpleTest_PutGetManyKeys) {
        test_put_get(1000000, h, true);
    }

    TEST_F(Silt_Cuckoo_Test, SimpleTest_PutUndoGet) {
        test_put_undo_get(h);
    }

    TEST_F(Silt_Cuckoo_Test, SimpleTest_Loadfactor) {
        test_loadfactor(h);
    }
