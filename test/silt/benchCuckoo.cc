...
void benchmark(std::string name)
{
...

	{
		...
		for (size_t i = 0; i < 1024; i++) {
            Silt* silt = Silt_Factory::New(new Configuration(config));
            silt->Create();

			size_t j;
			for (j = 0; j < size; j++) {
				if (silt->Put(arr[j].key, RefValue(&j)) != OK)
					break;
			}
            index_size = j;
			insert_count += j;

            indexes.push_back(silt);
		}
    ...
    for (size_t lookup_mode = 0; lookup_mode < 2; lookup_mode++)
	{
		...
		for (size_t k = 0; k < lookup_count; k++)
        {
            ...
            if (lookup_mode == 0)
            {
                Silt_ConstIterator it = indexes[i]->Find(arr[j].key);
                while (!it.IsEnd()) {
            ...
            {
                Silt_ConstIterator it = indexes[i]->Find(arr2[j].key);
                while (!it.IsEnd()) {
                    ++it;
                }
            }
