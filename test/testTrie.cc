...
generate_random_kv(kv_array_type& out_arr, size_t key_len, size_t data_len, size_t size, unsigned int seed = 0)
{
    ...
    for (size_t i = 0; i < size; i++) {
		kv_pair kv;
		kv.key = silt::RefValue(buf + i * kv_len, key_len);
		kv.data = silt::RefValue(buf + i * kv_len + key_len, data_len);
...
namespace silt
{
    ...
    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleInsertRetrieve1) {
        EXPECT_EQ(OK, silt_->Put(arr_[0].key, arr_[0].data));
        EXPECT_EQ(OK, silt_->Flush());

        EXPECT_EQ(OK, silt_->Get(arr_[0].key, ret_data_));
        EXPECT_EQ(data_len_, ret_data_.size());
        EXPECT_EQ(0, memcmp(arr_[0].data.data(), ret_data_.data(), data_len_));
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleInsertRetrieve2) {
        arr_[0].key.data()[0] = 5;
        arr_[1].key.data()[0] = 7;

        EXPECT_EQ(OK, silt_->Put(arr_[0].key, arr_[0].data));
        EXPECT_EQ(OK, silt_->Put(arr_[1].key, arr_[1].data));
        EXPECT_EQ(OK, silt_->Flush());
        ...
        EXPECT_EQ(OK, silt_->Get(arr_[1].key, ret_data_));
        EXPECT_EQ(data_len_, ret_data_.size());
        EXPECT_EQ(0, memcmp(arr_[1].data.data(), ret_data_.data(), data_len_));
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleUnsortedInsertRetrieve4) {
        arr_[0].key.data()[0] = 5;
        arr_[1].key.data()[0] = 7;
        arr_[2].key.data()[0] = 3;
        arr_[3].key.data()[0] = 9;
        ...
        EXPECT_EQ(OK, silt_->Put(arr_[0].key, arr_[0].data));
        EXPECT_EQ(OK, silt_->Put(arr_[1].key, arr_[1].data));
        EXPECT_EQ(INVALID_KEY, silt_->Put(arr_[2].key, arr_[2].data));
        EXPECT_EQ(OK, silt_->Put(arr_[3].key, arr_[3].data));
        EXPECT_EQ(OK, silt_->Flush());
        ...
        EXPECT_EQ(KEY_NOT_FOUND, silt_->Get(arr_[2].key, ret_data_));

        EXPECT_EQ(OK, silt_->Get(arr_[3].key, ret_data_));
        EXPECT_EQ(data_len_, ret_data_.size());
        EXPECT_EQ(0, memcmp(arr_[3].data.data(), ret_data_.data(), data_len_));
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleSortedInsertRetrieve4) {
        arr_[0].key.data()[0] = 3;
        arr_[1].key.data()[0] = 5;
        arr_[2].key.data()[0] = 7;
        arr_[3].key.data()[0] = 9;
        ...
        EXPECT_EQ(OK, silt_->Put(arr_[0].key, arr_[0].data));
        EXPECT_EQ(OK, silt_->Put(arr_[1].key, arr_[1].data));
        EXPECT_EQ(OK, silt_->Put(arr_[2].key, arr_[2].data));
        EXPECT_EQ(OK, silt_->Put(arr_[3].key, arr_[3].data));
        EXPECT_EQ(OK, silt_->Flush());
        ...
        EXPECT_EQ(OK, silt_->Get(arr_[2].key, ret_data_));
        EXPECT_EQ(data_len_, ret_data_.size());
    ...
    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleSortedInsertRetrieveSome) {
        sort_keys(arr_, key_len_, 0, 100);

        for (size_t i = 0; i < 100; i++)
            EXPECT_EQ(OK, silt_->Put(arr_[i].key, arr_[i].data));
        EXPECT_EQ(OK, silt_->Flush());

        for (size_t i = 0; i < 100; i++)
        {
            EXPECT_EQ(OK, silt_->Get(arr_[i].key, ret_data_));
            EXPECT_EQ(data_len_, ret_data_.size());
            EXPECT_EQ(0, memcmp(arr_[i].data.data(), ret_data_.data(), data_len_));
        }
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleSortedInsertRetrieveMany) {
		...
		{
			Value status;
			if (silt_->Status(MEMORY_USE, status) == OK)
...
//    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleSortedInsertRetrieveManyReloaded) {
//        sort_keys(arr_, key_len_, 0, size_);
//
//        for (size_t i = 0; i < size_; i++)
//            EXPECT_EQ(0, silt_->Insert(arr_[i], key_len_, arr_[i] + key_len_, data_len_));
//        EXPECT_EQ(0, silt_->Flush());
//
//        delete silt_;
//        silt_ = Silt_Factory::Open_Silt(conf_file);
//
//        for (size_t i = 0; i < size_; i++)
//        {
//            EXPECT_EQ(0, silt_->Get(arr_[i], key_len_, ret_data_, ret_data_len_));
//            EXPECT_EQ(data_len_, ret_data_len_);
//            EXPECT_EQ(0, memcmp(arr_[i] + key_len_, ret_data_, ret_data_len_));
//        }
//    }
//
    TEST_F(Silt_SF_Ordered_Trie_Test, TestSimpleIterator) {
        sort_keys(arr_, key_len_, 0, size_);
        ...
        size_t idx = 0;
		for (Silt_ConstIterator it = silt_->Enumerate(); !it.IsEnd(); ++it)
		{
    ...
    TEST_F(Silt_SF_Ordered_Trie_Test, TestGetBeforeFinalizing) {
        sort_keys(arr_, key_len_, 0, size_);

        EXPECT_EQ(OK, silt_->Put(arr_[0].key, arr_[0].data));
        EXPECT_EQ(ERROR, silt_->Get(arr_[0].key, ret_data_));
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestInsertAfterFinalizing) {
        sort_keys(arr_, key_len_, 0, size_);

        EXPECT_EQ(OK, silt_->Flush());
        EXPECT_EQ(ERROR, silt_->Put(arr_[0].key, arr_[0].data));
    }

    TEST_F(Silt_SF_Ordered_Trie_Test, TestFlushAfterFinalizing) {
        sort_keys(arr_, key_len_, 0, size_);
...
//    TEST_F(Silt_SF_Ordered_Trie_Test, TestInsertAfterReloading) {
//        sort_keys(arr_, key_len_, 0, size_);
//
//        EXPECT_EQ(0, silt_->Insert(arr_[0], key_len_, arr_[0] + key_len_, data_len_));
//        EXPECT_EQ(0, silt_->Flush());
//
//        delete silt_;
//        silt_ = Silt_Factory::Open_Silt(conf_file);
//
//        EXPECT_EQ(-1, silt_->Insert(arr_[1], key_len_, arr_[1] + key_len_, data_len_));
//    }
//
//    TEST_F(Silt_SF_Ordered_Trie_Test, TestFlushAfterReloading) {
//        sort_keys(arr_, key_len_, 0, size_);
//
//        EXPECT_EQ(0, silt_->Insert(arr_[0], key_len_, arr_[0] + key_len_, data_len_));
//        EXPECT_EQ(0, silt_->Flush());
