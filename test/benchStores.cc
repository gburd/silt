...
std::vector<Silt*> g_log_stores;
std::vector<Silt*> g_hash_stores;
std::vector<Silt*> g_sorted_stores;
...
void* constructor_thread_main(void* arg)
{
    ...
    while (true) {
        ...
        Silt* silt = Silt_Factory::New(store_config);
        assert(silt);
        ...
        if (g_phase == 0) {
            for (size_t j = 0; j < g_size; j++) {
                ...
                if (silt->Put(ConstRefValue(key, g_key_len), ConstRefValue(data, g_data_len)) != OK)
                    assert(false);
            }
            g_log_stores[current_store_i] = silt;
        }
        else if (g_phase == 1) {
            if (g_log_stores[current_store_i]->ConvertTo(silt) != OK)
                assert(false);
            ...
            g_hash_stores[current_store_i] = silt;
        }
        else {
            // TODO: this code is largely borrowed from Silt_Combi;
            // it would be probably useful to make Silt more flexible to expose this sorting functionally to the external.
            Silt* sorter;
            ...
            for (size_t j = 0; j < g_merge_size; j++) {
                if (current_store_i * g_merge_size + j >= g_hash_stores.size())
                    break;
                Silt_ConstIterator it = g_hash_stores[current_store_i * g_merge_size + j]->Enumerate();
                while (!it.IsEnd()) {
                    Value combined_data;
            ...
            g_sorted_stores[current_store_i] = silt;
        }
    ...
    while (true) {
        ...
        for (size_t trial = 0; trial < get_batch_size; trial++) {
            ...
            if (g_phase == 0) {
                ...
                silt = g_hash_stores[i];
            }
            else {
                assert(i / g_merge_size < g_sorted_stores.size());
                silt = g_sorted_stores[i / g_merge_size];
            }
            assert(silt);

            Silt_Return ret = silt->Get(ConstRefValue(key, g_key_len), data);
            Silt_Return expected_ret;
            if (g_successful_lookup)
                expected_ret = OK;
            else
