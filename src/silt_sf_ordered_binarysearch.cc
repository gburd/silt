...
namespace silt
{
    ...
    Silt_SF_Ordered_BinarySearch<Store>::Create(Configuration* const &config, const uint64_t &hash_table_size)
    {
        Silt_SF_Ordered_BinarySearch<Store>* silt = new Silt_SF_Ordered_BinarySearch<Store>(config);
        silt->Initialize(hash_table_size);
        return silt;
    }
    ...
    template <typename Store>
    Silt_SF_Ordered_BinarySearch<Store>::Silt_SF_Ordered_BinarySearch(Configuration* const &config)
    ...
    template <typename Store>
    Silt_SF_Ordered_BinarySearch<Store>::~Silt_SF_Ordered_BinarySearch()
    {
    ...
    template <typename Store>
    void
    Silt_SF_Ordered_BinarySearch<Store>::Initialize(uint64_t hash_table_size)
    {
        if (initialized_) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Initialize: reinitailization\n");
            return;
        }
        ...
        initialized_ = true;
        fprintf(stdout, "Silt_SF_Ordered_BinarySearch: Initialize: initialized\n");
    }

    template <typename Store>
    void
    Silt_SF_Ordered_BinarySearch<Store>::LoadFromFile()
    {
        if (initialized_) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: LoadFromFile: already initialized\n");
            return;
        }
        ...
        if (read(fd, &hash_table_size_, sizeof(hash_table_size_)) != sizeof(hash_table_size_)) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: LoadFromFile: unable to read hash table size\n");
            close(fd);
            return;
        }
        ...
        if (read(fd, &actual_size_, sizeof(actual_size_)) != sizeof(actual_size_)) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: LoadFromFile: unable to read actual hash table size\n");
            close(fd);
            return;
        }
        ...
        initialized_ = true;
        fprintf(stdout, "Silt_SF_Ordered_BinarySearch: LoadFromFile: loaded\n");

#ifdef DEBUG
        fprintf(stdout, "Silt_SF_Ordered_BinarySearch: LoadFromFile: state: %lu\n", hash_state());
#endif
    }

    template <typename Store>
    void
    Silt_SF_Ordered_BinarySearch<Store>::StoreToFile()
    {
        if (!initialized_) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: StoreToFile: not initailized\n");
            return;
        }

        if (!finalized_) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: StoreToFile: not finalized\n");
            return;
        }
        ...
        if ((fd = open(filename.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_NOATIME, 0666)) == -1) {
            perror("Silt_SF_Ordered_BinarySearch: StoreToFile: could not open index file");
            return;
        }

        // write base offset
        if (write(fd, &base_data_store_offset_, sizeof(base_data_store_offset_)) != sizeof(base_data_store_offset_)) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: StoreToFile: unable to write base offset\n");
            close(fd);
            return;
        }
        ...
        if (write(fd, &hash_table_size_, sizeof(hash_table_size_)) != sizeof(hash_table_size_)) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: StoreToFile: unable to write hash table size\n");
            close(fd);
            return;
        }
        ...
        if (write(fd, &actual_size_, sizeof(actual_size_)) != sizeof(actual_size_)) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: StoreToFile: unable to write actual hash table size\n");
            close(fd);
            return;
        }

        close(fd);
        fprintf(stdout, "Silt_SF_Ordered_BinarySearch: StoreToFile: stored\n");

#ifdef DEBUG
        fprintf(stdout, "Silt_SF_Ordered_BinarySearch: StoreToFile: state: %lu\n", hash_state());
#endif
    }
                                    ...
                                    const uint32_t &data_len)
    {
        ...
        if (debug_level & 2) {
            if (key_len != key_len_)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: invalid key_len\n");
            if (data_len != data_len_)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: invalid data_len\n");
            if (key == NULL)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: key is NULL!\n");
            if (data == NULL)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: data is NULL!\n");
        }
#endif

        DPRINTF(2, "Silt_SF_Ordered_BinarySearch: Called Insert for key with length: %i at mem: %i and data: %s with length: %i at mem: %i ...", key_len, key, data, data_len, data);
        ...
        if (finalized_)
        {
            ...
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: already finalized\n");
            return -1;
        }
        ...
        {
            pthread_rwlock_unlock(&silt_lock_);
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: duplicate or not sorted key\n");
            return -1;
        }
        ...
        if (data_store_->Put(data_store_offset, key_len, data_len, key, data) != 0) {
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Insert: failed to Put to datastore\n");
            pthread_rwlock_unlock(&silt_lock_);
            return -1;
        }
    ...
    Silt_SF_Ordered_BinarySearch<Store>::Get(const char* const &key, const uint32_t &key_len, char* &data, uint32_t &data_len)
    {
        ...
        if (debug_level & 2) {
            if (key_len != key_len_)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Get: invalid key_len\n");
            if (key == NULL)
                fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Get: key is NULL!\n");
        }
#endif

        DPRINTF(2, "Silt_SF_Ordered_BinarySearch: Called Get for key with length: %i at mem: %i ...", key_len, key);
    ...
    template <typename Store>
    int
    Silt_SF_Ordered_BinarySearch<Store>::Delete(const char* const &key __attribute__ ((unused)), const uint32_t &key_len __attribute__ ((unused)))
    {
        fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Delete: not supported\n");
        return -1;
    }
    ...
    template <typename Store>
    int
    Silt_SF_Ordered_BinarySearch<Store>::DoForEach(void (*callForEach)(const uint32_t &key_len, const uint32_t &data_len, const char* const &key, const char* const &data, const bool &isDeleted), void (*callBeforeRest)()) const
    {
    ...
    */
    ...
    Silt_SF_Ordered_BinarySearch<Store>::Flush()
    {
        ...
        if (finalized_) {
            pthread_rwlock_unlock(&silt_lock_);
            fprintf(stderr, "Silt_SF_Ordered_BinarySearch: Flush: already finalized\n");
            return -1;
        }
    ...
    template <typename Store>
    SiltIter
    Silt_SF_Ordered_BinarySearch<Store>::IteratorBegin() const {
        return data_store_->IteratorBegin();
    }

    template <typename Store>
    std::size_t
    Silt_SF_Ordered_BinarySearch<Store>::lookup_key_index(const char* key, uint64_t off, int64_t n) const
    {
    ...
    template <typename Store>
    void
    Silt_SF_Ordered_BinarySearch<Store>::finalize()
    {
        finalized_ = true;
    ...
    template <typename Store>
    std::size_t
    Silt_SF_Ordered_BinarySearch<Store>::hash_state() const
    {
        std::size_t sum = 0;
