...
namespace silt {
    ...
    {
        pthread_rwlock_init(&silt_lock_, NULL);
    }
    ...
    Silt_SF::~Silt_SF()
    {
        ...
        pthread_rwlock_destroy(&silt_lock_);
    }

    Silt_Return
    Silt_SF::Create()
    {
        DPRINTF(2, "Silt_SF::Create(): creating header\n");
        ...
        //silt->header_->max_deleted_ratio = MAX_DELETED_RATIO;
        //silt->header_->max_load_factor = MAX_LOAD_FACTOR;
        DPRINTF(2, "Silt_SF::Create(): creating hash table\n");
        ...
        hash_table_ = Silt_Factory::New(tableConfig);
        ...
        DPRINTF(2, "Silt_SF::Create(): creating data store\n");
        ...
        if (data_store_ == NULL) {
            DPRINTF(2, "Silt_SF::Create(): could not create data store\n");
            return ERROR;
        }
        ...
        DPRINTF(2, "Silt_SF::Create(): <result> done\n");
        return OK;
    }
    ...
    Silt_SF::Open()
    {
        ...
        DPRINTF(2, "Silt_SF::Open(): reading header\n");

        if (ReadHeaderFromFile() != 0) {
            perror("Could not read header from file\n");
            printf("Populating Silt from file failed\n");
            return ERROR;
        }

        DPRINTF(2, "Silt_SF::Open(): opening hash table\n");
        Configuration *tableConfig = new Configuration(config_, true);
        tableConfig->SetContextNode("child::hashtable");
        tableConfig->SetStringValue("child::id", config_->GetStringValue("child::id"));
        hash_table_ = Silt_Factory::New(tableConfig);
        ...
        DPRINTF(2, "Silt_SF::Open(): opening data store\n");
        Configuration *storeConfig = new Configuration(config_, true);
        storeConfig->SetContextNode("child::datastore");
        storeConfig->SetStringValue("child::id", config_->GetStringValue("child::id"));
        ...
        DPRINTF(2, "Silt_SF::Open(): <result> done\n");
        return OK;
    }

    Silt_Return
    Silt_SF::ConvertTo(Silt *new_store) const
    {
        pthread_rwlock_rdlock(&silt_lock_);
        Silt_SF *sf = dynamic_cast<Silt_SF *>(new_store);
        ...
        if (key_len_ != sf->key_len_) {
            fprintf(stderr, "Silt_SF::ConvertTo(): key length mismatch\n");
            pthread_rwlock_unlock(&silt_lock_);
            return ERROR;
        }

        if (data_len_ != sf->data_len_) {
            fprintf(stderr, "Silt_SF::ConvertTo(): data length mismatch\n");
            pthread_rwlock_unlock(&silt_lock_);
            return ERROR;
        }

        if (hash_table_->ConvertTo(sf->hash_table_) != OK) {
            fprintf(stderr, "Silt_SF::ConvertTo(): could not convert hash table\n");
            pthread_rwlock_unlock(&silt_lock_);
            return ERROR;
        }

        //fprintf(stderr, "Silt_SF::ConvertTo(): analyzing hash table changes\n");
        ...
        if (sf->config_->ExistsNode("child::keep-datastore-on-conversion") == 0 &&
            ...
            if (data_store_->ConvertTo(sf->data_store_) != OK) {
                fprintf(stderr, "Silt_SF::ConvertTo(): could not convert data store\n");
                pthread_rwlock_unlock(&silt_lock_);
                return ERROR;
            }
        ...
        Silt_Iterator from_it = hash_table_->Enumerate();
        Silt_Iterator to_it = sf->hash_table_->Enumerate();
        ...
        while (true) {
            if (!from_it.IsEnd() && !to_it.IsEnd()) {
                ...
                //fprintf(stderr, "Silt_SF::ConvertTo(): mapping %zu => %zu\n", static_cast<size_t>(from), static_cast<size_t>(to));
        ...
        Silt_ConstIterator it_from = data_store_->Enumerate();
        ...
        for (it = mapping.begin(); it != mapping.end(); ++it) {
            ...
            Silt_Return ret = sf->data_store_->Put(RefValue(&to), it_from->data);
        ...
        */
        ...
        for (it = rev_mapping.begin(); it != rev_mapping.end(); ++it) {
            ...
            while (++last_to != to) {
                Silt_Return ret = sf->data_store_->Put(RefValue(&last_to), data);
                if (ret != OK) {
                    pthread_rwlock_unlock(&silt_lock_);
                    return ERROR;
                }
        ...
        */
        //fprintf(stderr, "Silt_SF::ConvertTo(): flushing data store\n");
        Silt_Return ret = sf->data_store_->Flush();
    ...
    Silt_SF::Flush()
    {
        pthread_rwlock_wrlock(&silt_lock_);
    ...
    Silt_SF::Close()
    {
        ...
        DPRINTF(2, "Silt_SF::Close(): closing\n");
        pthread_rwlock_wrlock(&silt_lock_);
        delete header_;
        header_ = NULL;
        delete hash_table_;
    ...
    Silt_SF::Destroy()
    {
        ...
        tableConfig->SetContextNode("child::hashtable");
        tableConfig->SetStringValue("child::id", config_->GetStringValue("child::id"));
        Silt *hash_table = Silt_Factory::New(tableConfig);
        Silt_Return ret_destroy_hash_table = hash_table->Destroy();
        delete hash_table;
        Configuration *storeConfig = new Configuration(config_, true);
        storeConfig->SetContextNode("child::datastore");
        storeConfig->SetStringValue("child::id", config_->GetStringValue("child::id"));
    ...
    Silt_Return
    Silt_SF::Status(const Silt_StatusType &type, Value &status) const
    {
        if (!header_)
            return ERROR;
    ...
    Silt_Return
    Silt_SF::Put(const ConstValue &key, const ConstValue &data)
    {
#ifdef DEBUG

        if (debug_level & 2) {
            DPRINTF(2, "Silt_SF::Put(): key=\n");
            print_payload((const u_char *)key.data(), key.size(), 4);
            DPRINTF(2, "Silt_SF::Put(): data=\n");
    ...
    Silt_Return
    Silt_SF::Delete(const ConstValue &key)
    {
#ifdef DEBUG

        if (debug_level & 2) {
            DPRINTF(2, "Silt_SF::Delete(): key=\n");
            print_payload((const u_char *)key.data(), key.size(), 4);
        }
    ...
    Silt_Return
    Silt_SF::InsertEntry(const ConstValue &key, const ConstValue &data, bool isDelete)
    {
        assert(!isDelete || (isDelete && data.size() == 0));
        pthread_rwlock_wrlock(&silt_lock_);
        bool newObj = true;
        Silt_Iterator hash_it = hash_table_->Find(key);
        size_t dhSize;
        ...
        while (!hash_it.IsEnd()) {
            ...
            if (data_store_->Get(data_store_key, read_hdr, 0, dhSize) != OK ||
                ...
                DPRINTF(2, "Silt_SF::InsertEntry(): key length mismatch; continue\n");
                ++hash_it;
                continue;
            }

            DPRINTF(2, "Silt_SF::InsertEntry(): key length match\n");
            SizedValue<64> read_key;

            if (data_store_->Get(data_store_key, read_key, dhSize, key.size()) != OK ||
                key != read_key) {
                DPRINTF(2, "Silt_SF::InsertEntry(): key mismatch; continue\n");
                ++hash_it;
                continue;
            }

            DPRINTF(2, "Silt_SF::InsertEntry(): key match\n");
            newObj = false;
            break;
        }

        if (hash_it.IsEnd()) {
            DPRINTF(2, "Silt_SF::InsertEntry(): key not found in hash table\n");
        }
        ...
        Silt_Return ret_append = data_store_->Append(entry_id, buf);

        if (ret_append != OK) {
            DPRINTF(2, "Silt_SF::InsertEntry(): <result> failed to write entry\n");
            pthread_rwlock_unlock(&silt_lock_);
            return ret_append;
        }

        DPRINTF(2, "Silt_SF::InsertEntry(): inserted entry ID=%llu\n", entry_id.as_number<long long unsigned>());
        // update the hashtable (since the data was successfully written).
        assert(entry_id.as_number<size_t>(-1) != static_cast<size_t>(-1));

        if (hash_it.IsEnd()) {
            Silt_Return ret_put = hash_table_->Put(key, entry_id);

            if (ret_put != OK) {
                DPRINTF(2, "Silt_SF::InsertEntry(): <result> failed to add to hashtable\n");
                pthread_rwlock_unlock(&silt_lock_);
            ...
            Silt_Return ret_replace = hash_it->Replace(entry_id);

            if (ret_replace != OK) {
                DPRINTF(2, "Silt_SF::InsertEntry(): <result> failed to update hashtable\n");
                pthread_rwlock_unlock(&silt_lock_);
        ...
        DPRINTF(2, "Silt_SF::InsertEntry(): <result> updated hashtable\n");
        pthread_rwlock_unlock(&silt_lock_);
    ...
    Silt_Return
    Silt_SF::Contains(const ConstValue &key) const
    {
        if (key.size() == 0)
            return INVALID_KEY;
        ...
        size_t len = 0;
        Silt_Return ret = Length(key, len);
    ...
    Silt_Return
    Silt_SF::Length(const ConstValue &key, size_t &len) const
    {
#ifdef DEBUG

        if (debug_level & 2) {
            DPRINTF(2, "Silt_SF::Length(): key=\n");
            print_payload((const u_char *)key.data(), key.size(), 4);
        }
        ...
        if (debug_level & 2)
            DPRINTF(2, "Silt_SF::Length(): <result> len=%zu\n", len);
    ...
    Silt_Return
    Silt_SF::Get(const ConstValue &key, Value &data, size_t offset, size_t len) const
    {
        if (key.size() == 0)
            return INVALID_KEY;
        ...
        while (!hash_it.IsEnd()) {
            ...
            if (data_store_->Get(data_store_key, read_hdr, 0, dhSize) != OK ||
                (key_len_ == 0 && read_hdr.as<DataHeaderFull>().key_len != key.size())) {
                DPRINTF(2, "Silt_SF::Get(): key length mismatch; continue\n");
                ++hash_it;
                continue;
            }

            DPRINTF(2, "Silt_SF::Get(): key length match\n");
            SizedValue<64> read_key;

            if (data_store_->Get(data_store_key, read_key, dhSize, key.size()) != OK ||
                key != read_key) {
                DPRINTF(2, "Silt_SF::Get(): key mismatch; continue\n");
                ++hash_it;
                continue;
            }

            DPRINTF(2, "Silt_SF::Get(): key match\n");
            newObj = false;
            break;
        }

        if (newObj) {
            DPRINTF(2, "Silt_SF::Get(): <result> cannot find key\n");
            pthread_rwlock_unlock(&silt_lock_);
            return KEY_NOT_FOUND;
        }

        size_t total_len = 0;
        Silt_Return ret_len = data_store_->Length(data_store_key, total_len);
        ...
        if (type == 0) {
            ...
            DPRINTF(2, "Silt_SF::Get(): <result> deleted key\n");
            pthread_rwlock_unlock(&silt_lock_);
            return KEY_DELETED;
        }
        ...
        Silt_Return ret_data = data_store_->Get(data_store_key, data, dhSize + key.size() + offset, len);
#ifdef DEBUG

        if (debug_level & 2) {
            DPRINTF(2, "Silt_SF::Get(): <result> data=\n");
            print_payload((const u_char *)data.data(), data.size(), 4);
        }
    ...
    Silt_SF::IteratorElem::IteratorElem(const Silt_SF *silt)
    {
        this->silt = silt;
        pthread_rwlock_rdlock(&static_cast<const Silt_SF *>(silt)->silt_lock_);
    }

    Silt_SF::IteratorElem::~IteratorElem()
    {
        pthread_rwlock_unlock(&static_cast<const Silt_SF *>(silt)->silt_lock_);
    }

    Silt_IteratorElem *
    Silt_SF::IteratorElem::Clone() const
    {
        IteratorElem *elem = new IteratorElem(static_cast<const Silt_SF *>(silt));
        *elem = *this;
        return elem;
    }

    void
    Silt_SF::IteratorElem::Next()
    {
        Increment(false);
    }

    void
    Silt_SF::IteratorElem::Increment(bool initial)
    {
        while (true) {
            if (initial)
                initial = false;
            ...
            const Silt_SF *silt_sf = static_cast<const Silt_SF *>(silt);
    ...
    Silt_Return
    Silt_SF::WriteHeaderToFile() const
    {
